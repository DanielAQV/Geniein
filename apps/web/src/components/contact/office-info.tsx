    ...
    <div className="h-full flex flex-col md:flex-row lg:flex-col gap-6">
      {offices.map((office, index) => (
        <motion.div
          ...
          className={`flex-1 px-4 py-8 rounded-[20px] border backdrop-blur-md transition-all group shadow-md hover:shadow-lg flex flex-col justify-center cursor-pointer ${
            activeOffice === index
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              <div className="mt-1.5 h-4 w-4 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                <div
                  className={`h-1.5 w-1.5 rounded-full ${activeOffice === index ? "bg-primary" : "bg-primary/50"}`}
                />
              </div>
              <p className="text-sm text-muted-foreground break-keep leading-relaxed font-light">
                {office.address}
              </p>
            </div>
          </div>

          <div className="mt-3 border-t border-border/30 flex items-center justify-between">
