...
def _format(index: int, item: Evidence) -> str:
    ...
    # ★ `item.ocr_used` 는 일부러 내보내지 않는다.
    #
    #   예전에는 전사본마다 "스캔 원본을 옮긴 것이니 원본 확인을 권하세요" 를 붙였다.
    #   그러면 모델이 그 말을 거의 매 답변에 옮겨 적는다 — 코퍼스 대부분이 전사본이고
    #   대부분의 질문이 금액을 묻기 때문이다. 경고가 상시가 되면 아무도 안 읽는다.
    #
    #   불확실성 자체는 남아 있다. 되살릴 자리는 답변 문구가 아니라 **화면**이다 —
    #   근거 칩에 '전사본' 배지를 다는 쪽이 매 문장에 끼워 넣는 것보다 낫다.
    #   설계문서 5.1 이 말하는 원본 이미지 주입이 붙으면 그때 이 분기가 되살아난다.
