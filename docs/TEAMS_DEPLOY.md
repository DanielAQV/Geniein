...
> ★ 실제로 이 순서대로 하나씩 걸렸다. 문구가 바뀌는 것 자체가 진행 신호다:
> `로그인 정보를 받지 못했습니다`(SSO) → `아직 설정이 끝나지 않았습니다`(토큰 미설정)
> → `로그인이 만료되었습니다`(가드 401) → `검색 서버에 연결하지 못했습니다`(상류 없음)
> → 답변. 화면 문구만으로 어디까지 갔는지 알 수 있게 해둔 것이 진단 시간을 크게 줄였다.
...
#### ★★ 지금 인스턴스에는 뇌가 들어가지 않는다

```
t3.small — Mem 1.9Gi total / available 967Mi, swap 0, 2 vCPU
```

BGE-M3 는 568M 파라미터라 CPU fp32 로 올리면 **가중치만 2.3GB**, torch 오버헤드까지
3GB 를 넘는다. 그리고 이건 색인만의 문제가 아니다 — `kb/search.py` 의 질의 경로가
**임베딩을 무조건 호출한다** (하이브리드 검색의 벡터 브랜치. 어휘 검색만으로 떨어지는
폴백이 없다). 즉 **질의를 받으려면 서버 프로세스 안에 모델이 올라가 있어야 한다.**

스왑을 붙이면 뜨기는 하지만 질의마다 디스크를 긁어 실용성이 없다. t3.medium(4GB)도
기존 프로세스(홈페이지 Next + NestJS + genie 두 벌 + Postgres)가 이미 700MB 쯤 쓰므로
페이지 캐시 여유가 남지 않는다.

선택지: 인스턴스 상향(t3.large 이상) / 뇌 전용 인스턴스 분리 / 임베딩을 외부 API 로
교체(코퍼스 전체 재임베딩 + 교차언어 품질 재실측 필요).

#### 전 구간 검증 (2026-08-16)

인프라 결정 전에, 뇌를 **로컬 GPU 에 둔 채 터널로 열고** 서버의 `RAG_SERVICE_URL` 을
그쪽으로 돌려 전 구간을 확인했다. Teams SSO → BFF → NestJS(서비스 토큰 + Entra 가드)
→ 뇌 → 하이브리드 검색 → 조항·시행일이 붙은 답변까지 정상 동작했다.

검증용이므로 확인 직후 터널을 닫았다. 뇌를 인터넷에 두는 동안은 서비스 토큰이 유일한
자물쇠다 — 토큰 없이 `/agent/message` 가 401 로 막히는 것을 먼저 확인하고 열 것.
