...
**틀린 확장은 조용히 비싸다.** 엉뚱한 조항이 근거로 올라오고 모델은 그것을 성실하게 인용한다. 그래서 **뜻을 확인한 표기만** 적는다. 결재 어휘(`OTP`/`OTPR`/`PB`/`AL`)는 M365 구축 TF 채팅방 1,012건에서 **정의가 적힌 줄이 있는 것만** 옮겼다 — `"nhưng khi là OT payment request(OTPR)"` 같은 줄이다. 같은 방에서 쓰이지만 정의가 없는 `PR`·`PL`·`BO` 는 보류했다. 검색 로그에 `사전확장=[…]` 이 함께 남으므로, 근거가 이상할 때 사전이 범인인지 한 줄로 가려낼 수 있다.
