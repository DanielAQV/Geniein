...
1. ~~**Bot Framework 불필요 (확정).**~~ **2026-08-28 철회 — `docs/MIKEY_BOT.md`.** 전제가 "Teams 를 대화창으로 쓰지 않는다" 였는데, 마이키 탭이 실제 소통 창구가 되면서 그 전제가 깨졌다. 알림이 플로우봇에서 오면 채널이 갈리고, 마이키 이름으로 와서 **그 자리에서 되물을 수 있어야** 하나로 모인다. Graph 앱 전용 채팅 발신은 마이그레이션 용도로만 열려 있어 우회가 안 된다. 카드 게시만 필요한 경로에서는 여전히 Power Automate 로 충분하다는 판단은 유효하다.
...
**여러 낱말은 구(句)로 넣는다.** `OT payment request` 는 tsquery 인접 연산자로 `ot<->payment<->request` 가 되어 **붙어 있는** 세 낱말을 찾는다. 낱말로 쪼개 OR 로 넣으면 `request`·`plan`·`payment` 같은 흔한 단어가 홀로 검색어가 되어 수백 청크를 똑같이 긁고, 어휘 갈래의 순위가 평평해진다. 활용형은 표기마다 따로 적는다(`approval` / `approve` / `approved`) — 색인이 `simple` 사전이라 어간을 자르지 않아 셋이 서로 다른 토큰이다.

**사전은 재현율 장치이지 필터가 아니다.** 확장어가 늘어도 유효본·`role_scope`·`org_id` 필터는 갈래마다 그대로 걸린다. 이 파일이 인가 경계를 건드릴 수 있는 경로는 없다.

**틀린 확장은 조용히 비싸다.** 엉뚱한 조항이 근거로 올라오고 모델은 그것을 성실하게 인용한다. 그래서 **뜻을 확인한 표기만** 적는다. 결재 어휘(`OTP`/`OTPR`/`PB`/`AL`)는 M365 구축 TF 채팅방 1,012건에서 **정의가 적힌 줄이 있는 것만** 옮겼다 — `"nhưng khi là OT payment request(OTPR)"` 같은 줄이다. 채팅에 정의가 없던 `PR`(Purchase Request) / `PPL`(Purchasing Plan) / `PMR`(Payment Request) / `BO`(Business Operation) 은 담당자에게 확인한 뒤 넣었다.

**팀 이름은 한글 표기가 따로 있는 것만 넣는다.** `PMO` 처럼 한국인도 영어 철자를 그대로 쓰는 이름은 어휘 갈래가 이미 잡으므로 사전에 넣어도 새로 걸리는 게 없다. 반대로 `웨어하우스`·`인프라팀`·`어드민`·`엔지니어링` 은 직원이 한글로 적고 문서는 `Warehouse`·`BD Infra` 로 적어서, 그 사이가 지금까지 비어 있었다. 검색 로그에 `사전확장=[…]` 이 함께 남으므로, 근거가 이상할 때 사전이 범인인지 한 줄로 가려낼 수 있다.
...
| ~~신선도 경고~~ | **철회 (2026-08-27).** "오래된 문서이니 최신 여부를 확인하세요" 를 답변에 붙였더니 **거의 매 답변에 붙었다** — 코퍼스가 2018~2025년이라 대부분이 '오래된' 문서이고, 전사본 경고(`ocr_used`)도 같이 붙어서 답변 끝이 늘 주의문 두 줄이었다. 상시가 된 경고는 아무도 읽지 않고 답변만 길어진다. 시행일은 인용에 **사실로** 남기고(위 항목), 오래됐는지는 사용자가 그 날짜를 보고 판단한다. 되살린다면 답변 문구가 아니라 **근거 칩의 배지**로 — 읽는 사람이 필요할 때만 보게 |
...
⚠ **SharePoint REST 는 시크릿으로 받은 앱 전용 토큰을 거부한다.** 권한이 전부 맞아도 401 `Unsupported app only token` 이 온다 — 토큰의 `appidacr` 가 1(시크릿)이기 때문이고, SharePoint 는 2(인증서)만 받는다. 그래서 자격증명이 자원별로 갈린다: **Graph 는 시크릿, SharePoint 는 인증서**. 인증서는 서버에서 자체 서명으로 만들고 공개키만 앱 등록에 올린다 — 개인키는 뇌를 떠나지 않는다 (`GRAPH_CERT_PATH`, 파일 권한 600).
...
**방향 변경 (2026-08-28).** 카드 게시가 아니라 **봇 채널**로 간다. 이유와 절차는 `docs/MIKEY_BOT.md`.
- [x] `/bot/messages` 수신 + `/bot/notify` 발신 (`apps/api/src/bot/`)
- [x] `bot_conversations` — 손잡이 저장소 (설계문서 4장의 대화 로그와 별개)
- [x] manifest `bots` 블록 (`BOT_APP_ID` 가 있을 때만)
- [ ] **Azure Bot 등록** — AQV 테넌트 구독이 필요해서 막혀 있다
- [ ] Teams 클라이언트에서 실제 왕복 확인
- [ ] 카드 딥링크에 `?ref=` 컨텍스트 인계 (3.1) — 웹에 오면 마이키가 이미 그 건을 안다
