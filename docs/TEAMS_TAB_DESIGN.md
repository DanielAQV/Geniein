...
> **상태: 코드는 해결됨 (2026-08-16). 백필이 남았다.**
> RAG 세션이 커밋을 끝낸 뒤 `feat/yuna-agent-core` 를 머지하고 아래 필터를 붙였다.
> 다만 **기존 색인분 20건이 전부 `org_id = NULL`** 이라, 백필 전까지 검색은
> 전원 0건이다. 백필 값은 9장 1번(코퍼스 소유 법인)이 정해져야 확정된다.
...
#### ⚠ 남은 것: 백필 (실측 2026-08-16)

```
kb_documents = 20건 (org_id 채워진 것 0건, NULL 20건)
kb_chunks    = 232건
```

**전부 NULL이다. 백필 전까지 검색은 전원 0건이다** — 예상했던 그대로다.
색인된 20건은 한 법인 것으로 보인다 (`[Draft] AQV - Businesstrip SOP - 2025`,
`해외법인 시행세칙`, `Decision No. …/QĐTC…`). 설계문서 99행도 "AQV 테넌트"라 적고 있다.

```sql
-- 그 법인의 테넌트 UUID 를 넣고 1회 실행
UPDATE kb_documents SET org_id = '<tid>' WHERE org_id IS NULL;
```

**어느 UUID인지는 확인이 필요하다** — 틀리면 한쪽 직원 전체가 다른 회사 사규를
보게 되고, 그게 이 필터가 막으려던 바로 그 사고다. 9장 1번 참조.

백필 뒤 `③ 두 테넌트 계정으로 교차 검증`이 남는다. 이걸 건너뛰면
"0건이라 안전해 보이는" 상태와 "제대로 걸러서 안전한" 상태를 구분할 수 없다.
...
1. **★ 색인된 코퍼스는 어느 법인 것인가.** 4.4 백필의 `<tid>` 값. 문서 20건이
   AQV 계열로 보이는데, 에어키·지니 중 어느 테넌트인지가 확정돼야 한다.
   **틀리면 한쪽 직원 전체가 다른 회사 사규를 본다** — 이 필터를 붙인 이유 그 자체다.
   확정 전까지 검색은 0건이므로, Teams 화면 작업(커밋 5)의 실제 검증도 막혀 있다.
