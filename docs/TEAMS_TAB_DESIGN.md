...
4. **뇌를 사내 물리 서버에 두기로 했다 (2026-08-16).** EC2(t3.small)에는 BGE-M3 가
   들어가지 않고, 상향보다 이미 있는 물리 서버가 낫다는 판단이다. 그러면 새 문제가
   하나 생긴다 — **EC2 의 NestJS 가 사내 서버에 어떻게 닿는가.** 고정 터널 / VPN /
   사내 서버 공개 엔드포인트 중 하나를 정해야 하고, 어느 쪽이든 `RAG_SERVICE_URL` 이
   가리킬 **안정된 주소**여야 한다 (임시 터널은 재시작마다 바뀌어 쓸 수 없다).
   pgvector 와 `kb_*` 테이블, 사규 임베딩도 함께 그 서버로 옮긴다.

> privacy/terms 페이지는 만들지 않기로 했다 (2026-08-16). 조직 카탈로그 심사를
> 거치는 배포가 아니어서 요구되지 않는다. 매니페스트의 링크는 회사 사이트를 가리킨다.
...
| 2026-08-16 | 매니페스트를 **템플릿 + 빌드**로. 완성본을 커밋하지 않는다 | 안에 들어가는 값이 이미 `apps/api/.env` 에 있다. 두 벌을 두면 한쪽만 고치는 사고가 난다. 터널 검증도 `CONTENT_HOST` 하나로 끝난다 |
| 2026-08-16 | Teams 앱 `id` = Application (client) ID | 새 GUID 를 만들면 관리 대상이 하나 는다. 잃어버리면 Teams 가 다른 앱으로 인식해 사용자가 고정한 탭이 사라진다 |
