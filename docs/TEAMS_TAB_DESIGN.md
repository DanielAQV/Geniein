...
### 4.4.1 ★ 아직 열려 있는 문 — 유니크 인덱스가 org 로 안 나뉜다
...
1. **★ 지니 사규를 어떤 경로로 색인할 것인가.** 에어키(AQV) 문서를 복제해 이름만
   바꿔 쓸 계획인데, `kb_documents` 의 유니크 인덱스가 `(source, source_url)` 이라
   **org 로 나뉘어 있지 않다.** 같은 파일 경로를 다른 `--org-id` 로 재색인하면
   새 행이 생기는 게 아니라 기존 행의 `org_id` 가 갱신된다 — 에어키 코퍼스가
   통째로 지니로 넘어간다. 4.4.1 참조.
