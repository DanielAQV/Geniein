...
> **MVP가 검색 전용인 이유.** 에이전트 루프는 도구 연쇄 때문에 수십 초가 걸릴 수 있고
> (설계문서 3.4), 그러면 스트리밍 UX가 같이 필요해진다. 인증·iframe·배포 경로를 먼저
> 뚫는 게 목적이므로 응답이 1~2초로 끝나는 경로부터 붙인다.
...
**소유권**: `kb/search.py` · `tools/knowledge.py` 는 RAG 하이브리드 세션이 커밋 전
상태로 들고 있다. 이 브랜치에서 고치면 충돌한다. → **해당 세션에 이관**하고, 여기서는
`org_id` 가 걸린다는 것을 전제로 API 를 만든 뒤 통합 시점에 실제로 걸리는지 검증한다.

넘길 내용은 세 줄이다:

1. `search()` 에 `org_id: str | None` 추가
2. `_SCOPED` / `_LEX_CTE` **세 갈래 전부**에 `AND (d.org_id = %(org_id)s OR d.org_id IS NULL)` 추가
3. `knowledge.py` 가 받은 `org_id` 를 `search_kb` 로 전달
...
> ⚠ **백필이 선행한다.** 지금 색인된 문서는 `org_id` 가 전부 NULL 일 가능성이 높고,
> 그 상태로 이 조건이 붙으면 검색 결과가 **조용히 0건**이 된다. 통합 순서는
> ① 기존 문서 `org_id` 백필 → ② 필터 추가 → ③ 두 테넌트 계정으로 교차 검증.
> ③ 을 건너뛰면 "0건이라 안전해 보이는" 상태와 "제대로 걸러서 안전한" 상태를 구분할 수 없다.
